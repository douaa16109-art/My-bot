...
def generate_markup(chat_id, user_id):
    ...
    # زر الإضافي المستقل كما طلبتِ تماماً
...
def handle_buttons(call):
    ...
    if call.data == "reg_extra":
        ...
        bot.answer_callback_query(call.id, "تم تغيير حالة الإضافي")
        ...
        data['extra_roles'] = [p for p in data['extra_roles'] if p['id'] != uid]
        bot.answer_callback_query(call.id, "تم حذفك")
...
if __name__ == "__main__":
    keep_alive()
    bot.remove_webhook() # تنظيف التعليق
    time.sleep(1)
    bot.polling(none_stop=True, interval=0, timeout=20) # أقوى نمط تشغيل
