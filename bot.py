...
def get_user_rank(chat_id, user_id):
    try:
        ...
        # السماح للمالك والمشرفين برؤية الأزرار
...
def start_bot(m):
    if not get_user_rank(m.chat.id, m.from_user.id):
        bot.reply_to(m, "⚠️ عذراً، هذا الأمر لمشرفات المجلس فقط.")
        return 
...
def handle_buttons(call):
    ...
    if call.data == "choose_status":
        if not data['is_open']:
            bot.answer_callback_query(call.id, "⚠️ التسجيل مغلق حالياً.")
            return
    ...
    if call.data == "reg_read":
        ...
        data['readers'] = [p for p in data['readers'] if p['id'] != uid]
        data['listeners'] = [p for p in data['listeners'] if p['id'] != uid]
    elif is_admin and call.data == "toggle_lock":
        data['is_open'] = not data['is_open']
    elif is_admin and call.data == "admin_reset":
        data['readers'], data['listeners'] = [], []
    elif is_admin and call.data == "admin_set_surah":
        ...
        bot.edit_message_text("⚙️ إدارة الأسماء والترتيب:", cid, call.message.message_id, reply_markup=m)
        return
